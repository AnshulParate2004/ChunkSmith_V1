...
\begin{enumerate}
    ...
    \item \textbf{Layout Analysis}: We employ \textbf{YOLOv12} for high-precision layout detection. The model is fine-tuned to detect specific document elements, classifying regions into:
    \begin{itemize}
        \item \texttt{UncategorizedText}
        \item \texttt{PageNumber}
        \item \texttt{Table}
        \item \texttt{Footer}
        \item \texttt{Title}
        \item \texttt{Formula}
        \item \texttt{Image}
        \item \texttt{NarrativeText}
    \end{itemize}
...
This is the system's primary innovation. Raw text extraction from a complex chart is often gibberish. Instead, we pass the image of the chart to a Multimodal LLM with a \textbf{Structured Prompt}.
...
\end{itemize}
\begin{lstlisting}[language=Python, caption=The Semantic Enrichment Prompt, label=lst:prompt, basicstyle=\ttfamily\scriptsize\color{black}, keywordstyle=\color{black}, stringstyle=\color{black}]
...
We use a high-performance vector store (e.g., \textbf{Qdrant}) due to its performance characteristics and support for scalar quantization. The vector embedding is computed as:
\begin{equation}
