...
\author{\IEEEauthorblockN{1\textsuperscript{st} Anshul Parate}
\IEEEauthorblockA{\textit{ChunkSmith Project} \\
\textit{Github: AnshulParate2004/ChunkSmith}\\
India \\
anshulnparate@gmail.com}
}
\maketitle
\begin{abstract}
The paradigm of Retrieval-Augmented Generation (RAG) has fundamentally shifted the capabilities of Large Language Models (LLMs), allowing them to answer queries grounded in private, external data. However, a significant ``Modality Gap'' persists in enterprise definitions of RAG. Real-world documents—ranging from financial ledgers and scientific manuscripts to engineering schematics—encode a substantial portion of their specific knowledge in visual formats (charts, tables, diagrams). Traditional RAG pipelines, which predominantly rely on OCR-based text matching, strip away this visual context, reducing rich multimodal documents to flattened strings. This leads to hallucinations when models attempt to reason about visual trends or comparative data in tables.
To address this critical deficiency, we present \textbf{ChunkSmith}, a production-grade Multimodal RAG system engineered for high-fidelity visual context preservation. ChunkSmith introduces a novel \textit{Schema-First} content processing architecture that treats images and tables not as auxiliary metadata, but as distinct, first-class semantic entities. By leveraging state-of-the-art Multimodal LLMs (Gemini 2.5 Pro) and a purposely designed \textit{Structured Semantic Enrichment} strategy, the system generates dense vector representations that encode deep visual understanding and ``predicted question'' affordances. We further detail the system's distributed backend, which employs an asynchronous Round-Robin Key Rotation algorithm to effectively bypass provider rate limits, achieving a 4.6x improvement in ingestion throughput over serial baselines. Extensive qualitative and quantitative evaluations demonstrate ChunkSmith's superior ability to handle complex visual reasoning tasks while maintaining scalable performance metrics suitable for large corpora.
\end{abstract}
\begin{IEEEkeywords}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Visual Evidence Retrieval}: A user interaction model that retrieves and displays the \textit{source image} alongside the AI-generated answer for verification, enhancing trust and interpretability.
    \item \textbf{Production-Ready Implementation}: A complete system built with FastAPI, React, Qdrant, and Supabase, demonstrating the practical viability of the approach.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Object Storage (Supabase)}: Raw PDFs and extracted images are stored in cloud object storage for durability and accessibility.
    \item \textbf{Vector Database (Qdrant)}: Embeddings and metadata are stored in Qdrant for fast similarity search.
    \item \textbf{Relational Database}: Document metadata, processing status, and user information are stored in PostgreSQL (via Supabase).
\end{itemize}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Layout Analysis}: We employ \texttt{unstructured.io}'s partition API or custom heuristics to identify bounding boxes for distinct elements. The system detects:
    \begin{itemize}
        \item Text blocks (paragraphs, headings)
        \item Tables (identified by grid patterns)
        \item Images (embedded graphics, charts, diagrams)
        \item Equations (mathematical notation)
    \end{itemize}
...
\begin{equation}
Keep(I) = \begin{cases} 
1 & \text{if } Area(I) > \theta_{area} \land Aspect(I) \in [\phi_{min}, \phi_{max}] \\
0 & \text{otherwise}
\end{cases}
\end{equation}
...
\end{itemize}
Valid images are uploaded to Supabase Storage with a structured path:
\begin{verbatim}
...
This is the system's primary innovation. Raw text extraction from a complex chart is often gibberish. Instead, we pass the image of the chart to Gemini 2.5 Pro with a \textbf{Structured Prompt}.
...
class AIParser(BaseModel):
    ...
    summary: str = Field(
        description="Comprehensive summary of all data and information"
    )
    image_interpretation: List[str] = Field(
        description="Detailed analysis of each image"
    )
    table_interpretation: List[str] = Field(
        description="Row-column analysis of each table"
    )
\end{lstlisting}
This schema is passed to the MLLM using Gemini's structured output feature, which guarantees that the response will conform to the schema or fail explicitly (rather than returning malformed JSON).
...
\end{itemize}
\begin{lstlisting}[language=Python, caption=The Semantic Enrichment Prompt, label=lst:prompt]
...
- table_interpretation: Row-column analysis of tabular data
CONTENT TO ANALYZE:
TEXT CONTENT: {text}
TABLES: {tables}
...
We use \textbf{Qdrant} as the vector store due to its performance characteristics and support for scalar quantization. The vector embedding is computed as:
\begin{equation}
V_D = \mathcal{E}\left( T_{original} \oplus S_{AI} \oplus Q_{AI} \oplus I_{desc} \oplus T_{table} \right)
\end{equation}
Where:
\begin{itemize}[leftmargin=*]
    \item $\mathcal{E}$ is the embedding function (\texttt{gemini-embedding-001}, 768 dimensions)
...
\end{itemize}
The concatenation operator $\oplus$ represents string concatenation with appropriate delimiters. The final payload stored in Qdrant includes:
\begin{lstlisting}[language=Python, caption=Qdrant Payload Structure]
payload = {
    "chunk_index": int,
    "original_text": str,
    "ai_summary": str,
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Vector Database}: Qdrant Cloud
    \item \textbf{Object Storage}: Supabase Storage
    \item \textbf{MLLM Provider}: Google Gemini 2.5 Pro
    \item \textbf{Embedding Model}: gemini-embedding-001 (768 dimensions)
    \item \textbf{OCR}: Tesseract 5.0+
    \item \textbf{PDF Processing}: pypdf, pdf2image, Poppler
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Cloud Storage}: Supabase handles storage scaling automatically
    \item \textbf{Vector DB Sharding}: Qdrant supports distributed deployments
\end{itemize}
...
\bibitem{chen2022murag} J. Chen, et al., ``MuRAG: Multimodal Retrieval-Augmented Generator,'' \textit{arXiv preprint}, 2022.
